...
const refreshTokenCookieOptions: CookieOptions = {
  // Недоступны из JavaScript через свойства document.cookie. Очень важный момент.
  httpOnly: true,
  ...
  // Отсылаются на сервер только по протоколу SSL или HTTPS. Важный параметр для прода! (TODO: тут как раз поиграйся, что если ноденв продакшн, тогда тру, иначе фолс) - process.env.NODE_ENV === 'development' ? false : true
  // secure: true,
